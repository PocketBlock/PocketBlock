...
/**
 *
 * This file is modified for PocketBlock theme.
 *
 */
...
CustomDialog.show = function (title, message, options) {
    ...
    dialogDiv.innerHTML =
        ...
        (options.showInput ? '<div><input id="customDialogInput" class="form-control"></div>' : '') +
        '<div class="customDialogButtons" style="margin:10px">' +
        (options.showCancel ? '<button id="customDialogCancel" class="btn btn-danger" style="margin-right: 10px;">Cancel</button>' : '') +
        (options.showOkay ? '<button id="customDialogOkay" class="btn btn-success">OK</button>' : '') +
        '</div>';
